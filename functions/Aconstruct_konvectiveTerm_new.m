function [A,b]=Aconstruct_konvectiveTerm_new(A,b,Node_number_matrix,Coordinate_R,dz,dr)  
...
for ii=1:length(idx_P)
    A(idx_P(ii),idx_P(ii))=         1/(dz)              ;
    A(idx_P(ii),idx_S(ii))=         0                   ;
    A(idx_P(ii),idx_N(ii))=         0                   ;
    A(idx_P(ii),idx_W(ii))=         -1/(dz)             ;
